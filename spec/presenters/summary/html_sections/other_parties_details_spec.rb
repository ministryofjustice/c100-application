  ...
  describe HtmlSections::OtherPartiesDetails do
    ...
    describe '#answers' do
      ...
      it 'has the correct rows in the right order' do
        ...
        expect(answers[5].question).to eq(:other_party_refuge)
        expect(answers[5].change_path).to eq('/steps/other_party/refuge/uuid-123')
        expect(answers[5].value).to eq(GenericYesNoUnknown::NO.to_s)

        expect(answers[6]).to be_an_instance_of(AnswersGroup)
        expect(answers[6].name).to eq(:person_personal_details)
        expect(answers[6].change_path).to eq('/steps/other_party/personal_details/uuid-123')
        expect(answers[6].answers.count).to eq(3)
        ...
        expect(answers[8].name).to eq(:person_address_details)
        expect(answers[8].change_path).to eq('/steps/other_party/address_details/uuid-123')
        expect(answers[8].answers.count).to eq(1)
