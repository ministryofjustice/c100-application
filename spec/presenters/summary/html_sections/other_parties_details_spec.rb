  ...
  describe HtmlSections::OtherPartiesDetails do
    ...
    let(:other_party) {
      instance_double(OtherParty,
        ...
        are_identity_details_private: 'no',
    ...
    describe '#answers' do
      ...
      it 'has the correct rows in the right order' do
        ...
        expect(answers[4].question).to eq(:person_identity_details_private)
        expect(answers[4].change_path).to eq('/steps/other_party/identity_preferences/uuid-123')
        expect(answers[4].value).to eq('No')

        expect(answers[5]).to be_an_instance_of(FreeTextAnswer)
        expect(answers[5].question).to eq(:person_contact_details_private)
        expect(answers[5].change_path).to eq('/steps/other_party/privacy_preferences/uuid-123')
        expect(answers[5].value).to eq('No')

        expect(answers[6]).to be_an_instance_of(Answer)
        expect(answers[6].question).to eq(:other_party_refuge)
        expect(answers[6].change_path).to eq('/steps/other_party/refuge/uuid-123')
        expect(answers[6].value).to eq(GenericYesNoUnknown::NO.to_s)

        expect(answers[7]).to be_an_instance_of(AnswersGroup)
        expect(answers[7].name).to eq(:person_personal_details)
        expect(answers[7].change_path).to eq('/steps/other_party/personal_details/uuid-123')
        expect(answers[7].answers.count).to eq(3)
        ...
        expect(answers[8].change_path).to eq('/steps/other_party/relationship/uuid-123/child/uuid-555')
        expect(answers[8].value).to eq('mother')
        expect(answers[8].i18n_opts).to eq({child_name: 'Child Test'})

        expect(answers[9]).to be_an_instance_of(AnswersGroup)
        expect(answers[9].name).to eq(:person_address_details)
        expect(answers[9].change_path).to eq('/steps/other_party/address_details/uuid-123')
        expect(answers[9].answers.count).to eq(1)

          # personal_details group answers ###
          details = answers[9].answers
