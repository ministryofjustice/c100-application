...
RSpec.describe Steps::MiamExemptions::ExemptionUploadForm do

  let(:arguments) { {
    c100_application: c100_application,
    exemption_document: document_upload
  } }
  ...
  describe '#document_key' do
    it 'is correct' do
      expect(subject.document_key).to eq(:exemption)
    end
  end
  ...
  it_behaves_like 'a document attachable step form', attribute_name: :exemption
  ...
  describe '#save' do
    ...
    context 'when no documents uploaded' do
      ...
      it 'has validation error on the base' do
        ...
        expect(subject.errors[:exemption_document]).not_to be_empty
      end
    end
    ...
    context 'when a conflicting file is already uploaded' do
      ...
      it 'deletes any other documents if the document_key is :draft_consent_order and there is a exemption document' do
