...
describe Court do
  ...
  describe '#merge_from_json_lookup!' do
    ...
    it 'gets the Court with the given slug' do
      expect_any_instance_of(C100App::CourtfinderAPI).to receive(:court_lookup).with('my-slug').and_return(court)
    ...
    describe 'the court data' do
      context 'when it is nil' do
        before do
          allow_any_instance_of(C100App::CourtfinderAPI).to receive(:court_lookup).and_return(nil)
        end
      ...
      context 'when it includes a Court with a matching slug' do
        ...
        it 'sets opening_times to the hashes from the court data' do
          subject.send(:merge_from_json_lookup!)
          expect(subject.opening_times).to eq([{'description' => 'my description', 'hours' => '9:00-5pm'}])
        end
