When(/^I submit the attending court details$/) do
  ...
  expect(attending_court_language_page.content).to have_header
  attending_court_language_page.submit_language_requirements(
    language_interpreter_details: 'German needed for respondent'
  )

  expect(attending_court_special_arrangements_page).to be_displayed
  expect(attending_court_special_arrangements_page.content).to have_header
  attending_court_special_arrangements_page.submit_special_arrangements(
    separate_waiting_rooms: true,
  ...
  expect(attending_court_special_assistance_page.content).to have_header
...
Then(/^I should be taken to the attending court page$/) do
  ...
  expect(attending_court_intermediary_page.content).to have_header
end
