class ConsentOrderUploadPage < AnyPage
  include ActiveSupport::Testing::TimeHelpers
  set_url '/steps/opening/consent_order_upload'

  section :content, '#main-content' do
    element :header, 'h1', text: 'Upload the draft of your consent order'
    element :p_1, 'p', text: 'The draft of the consent order must be signed by you (the applicant) and the other person (respondent).'
    element :p_2, 'p', text: 'If you are uploading documents from a computer, name the files clearly. For example, consent order draft.pdf'
    element :p_3, 'p', text: 'Files must end with JPG, BMP, PNG, TIF, PDF, DOC or DOCX'
    ...
    element :p_4, 'p', text: 'Attach consent order draft as a single document'
    element :p_5, 'p', text: 'Your document will upload when you click \'Continue\'.'
    element :file_label, 'label', text: 'Select file'
